...
func TestParse_exact(t *testing.T) {
	require.Equal(t, vf.Value(32), tf.Parse(`32`))
	require.Equal(t, vf.Value(3.14), tf.Parse(`3.14`))
	require.Equal(t, vf.Value("pelle"), tf.Parse(`"pelle"`))
	require.Equal(t, tf.Tuple(vf.Value("pelle"), vf.Value(3.14), typ.Boolean), tf.Parse(`{"pelle", 3.14, bool}`))

	st := tf.Parse(`{"a":1..5,"b"?:2}`)
	require.Equal(t, tf.StructMap(false,
		tf.StructMapEntry(`a`, tf.IntegerRange(1, 5, true), true),
		tf.StructMapEntry(`b`, vf.Value(2), false)), st)
...
func TestParse_aliasBad(t *testing.T) {
	...
	require.Panic(t, func() { tf.Parse(`int=map[string](int|int)`) }, `attempt to redeclare identifier 'int'`)
}
...
func TestParse_unary(t *testing.T) {
	...
	require.Equal(t, typ.String, tf.Parse(`type[string]`))
	require.Equal(t, typ.Any, tf.Parse(`type`))
	require.Panic(t, func() { tf.Parse(`type[string`) }, `expected ']', got EOT`)
}

func TestParse_ternary(t *testing.T) {
	require.Equal(t, typ.Number, tf.Parse(`int|float`))
...
func TestParse_string(t *testing.T) {
	require.Equal(t, vf.String("\r"), tf.Parse(`"\r"`))
	require.Equal(t, vf.String("\n"), tf.Parse(`"\n"`))
	require.Equal(t, vf.String("\t"), tf.Parse(`"\t"`))
	require.Equal(t, vf.String("\""), tf.Parse(`"\""`))
