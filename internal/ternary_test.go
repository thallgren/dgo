...
func TestAllOfType(t *testing.T) {
	...
	require.Assignable(t, tp, `b`)
	require.Assignable(t, tp, `c`)
	...
	require.Equal(t, vf.Values("abc", "bca").(dgo.ArrayType).ElementType().(dgo.TernaryType).Operator(), dgo.OpAnd)
	...
	require.Assignable(t, tp, tf.AllOf(tf.Pattern(regexp.MustCompile(`a`)), vf.Value(`abc`)))
	require.NotAssignable(t, tp, tf.AllOf(tf.Pattern(regexp.MustCompile(`a`)), vf.Value(`abc`), typ.String))

	require.Assignable(t, tp, vf.Values("abc", "bca").(dgo.ArrayType).ElementType())
	require.NotAssignable(t, vf.Values("abc", "bca").(dgo.ArrayType).ElementType(), tp)
	require.Assignable(t, tp, tf.Parse(`"abc"^/a/&/b/&/c/`))
	require.Assignable(t, tf.Parse(`/a/|/b/|/c/`), tp)

	a := vf.Values(`a`, `b`, nil)
	require.Same(t, a.(dgo.ArrayType).ElementType(), a)
	require.Same(t, typ.Any, typ.Generic(a).(dgo.ArrayType).ElementType())
}

func TestAnyOfType(t *testing.T) {
	tp := tf.AnyOf(typ.Integer, typ.String)
	require.Assignable(t, tp, `hello`)
	require.Assignable(t, tp, 3)
	require.NotAssignable(t, tp, true)
...
func TestOneOfType(t *testing.T) {
	...
	require.Assignable(t, tp, `a`)
	require.Assignable(t, tp, 3)
	require.NotAssignable(t, tp, `ab`)
	require.NotAssignable(t, tp, true)
	...
	require.Assignable(t, tp, tf.OneOf(vf.String(`a`), vf.String(`b`), typ.Integer))
	require.NotAssignable(t, tp, tf.OneOf(vf.String(`ab`), typ.Integer))
...
func TestEnum(t *testing.T) {
	...
	require.Assignable(t, tp, `foo`)
	require.NotAssignable(t, tp, `oops`)
...
func TestIntEnum(t *testing.T) {
	...
	tp = tf.IntEnum(2, 4, 8)
	require.Assignable(t, tp, 4)
	require.NotAssignable(t, tp, 5)
